...
router.post('/clock/start', requireLogin, async (req, res) => {
  try {
    ...
    if (existingStart) {
      return res.redirect('/dashboard?clockError=' + encodeURIComponent('Bộ đếm đang chạy'));
    }
    ...
    res.redirect('/dashboard?clockError=' + encodeURIComponent('Không thể bắt đầu bộ đếm'));
  }
});
...
router.post('/clock/reset', requireLogin, async (req, res) => {
  try {
    ...
    if (existingEntry) {
      ...
      await Entry.create(userId, todayDate, true, 'Đặt lại bộ đếm - thất thủ', 2);
    }
    ...
    res.redirect('/dashboard?clockError=' + encodeURIComponent('Không thể đặt lại bộ đếm'));
  }
});
...
router.post('/clock/edit', requireLogin, async (req, res) => {
  try {
    ...
    if (daysNum < 0 || hoursNum < 0 || hoursNum > 23 || minutesNum < 0 || minutesNum > 59) {
      return res.redirect('/dashboard?clockError=' + encodeURIComponent('Giá trị thời gian không hợp lệ'));
    }

    // Check if clock is running
    const existingStart = await User.getClockStart(userId);
    if (!existingStart) {
      return res.redirect('/dashboard?clockError=' + encodeURIComponent('Bộ đếm chưa chạy'));
    }
    ...
    res.redirect('/dashboard?clockError=' + encodeURIComponent('Không thể chỉnh bộ đếm'));
  }
});
